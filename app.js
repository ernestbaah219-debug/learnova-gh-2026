...
function tutorDirectMath(q){
 ...
 if(!Number.isFinite(r))return '<b>AI Tutor 5.0</b><br><br>I can work this out, but division by zero is not defined. Please check the numbers.';
 return `<b>AI Tutor 5.0 • Step-by-step solution</b><br><br><b>1. Identify:</b> We need to calculate ${a} ${esc(op)} ${b}.<br><b>2. Work it out:</b> ${a} ${esc(op)} ${b} = <b>${r}</b>.<br><b>3. Check:</b> The result is ${r}.<br><br><b>Answer: ${r}</b><br><br>Send the full question if there are more steps, units or a word problem.`;
}
...
function uniqueExamQuestions(l,s,t,count){
 ...
 while(out.length<count&&i<80){const seed=(seedBase+i*2654435761)>>>0;let question,answer;
  if(isMath(s)){
   ...
   const v=verbs[i%verbs.length];question=`${l}: ${v} ${t.title} in a ${['school','community','Ghanaian','real-world','classroom'][i%5]} situation (version ${i+1}).`;answer=i%2===0?def:`A correct answer must use the main idea of ${t.title}, relevant evidence or an appropriate application.`;
  }
...
function renderSchoolPortal(){
 ...
 if(r==="parent"){
   ...
   document.getElementById("parentSummary").textContent=avg>=80?"Excellent progress. Encourage continued challenge and revision.":avg>=55?"Progress is developing. Encourage regular practice and revision.":"The learner needs more foundation practice. Use AI Tutor 5.0 and Smart Revision.";
 }
...
function advancedMathTutor(q,l,s,t){
 ...
 if(/^[0-9+*/().%^×÷−\-\s]+$/.test(expr) && /\d/.test(expr)){
  ...
  if(r!==null)return `<b>AI Tutor 5.0 • Step-by-step Maths</b><br><br><b>Question:</b> ${esc(raw)}<br><b>Step 1:</b> Follow the order of operations: brackets, powers, multiplication/division, then addition/subtraction.<br><b>Step 2:</b> Calculate carefully: <b>${esc(expr)}</b>.<br><b>Step 3:</b> Check the result by substituting or estimating.<br><br><b>Answer: ${mathFormat(r)}</b>`;
 }
...
function genericTutorAnswer(q,l,s,t){
 ...
 if(topicObj){const def=topicDefinition(topicObj),examples=topicObj.examples||[],ex=examples[0]||`Apply ${title} to a real ${subject} situation.`, lower=String(q).toLowerCase();
  if(/\b(what is|what are|define|meaning|who is|explain)\b/.test(lower))return `<b>AI Tutor 5.0 • ${esc(l)}</b><br><br><b>Definition:</b> ${esc(def)}<br><br><b>Detailed explanation:</b><br>${textbookExplanation(topicObj,l,subject)}<br><br><b>Topic-matched example:</b><br>${esc(ex)}`;
  if(/\b(why|importance|important|reason|purpose|benefit)\b/.test(lower))return `<b>AI Tutor 5.0</b><br><br><b>Why ${esc(title)} is important</b><br>${esc(title)} is important because it helps learners understand and apply ideas in ${esc(subject)}. ${rank>=10?'At SHS level, connect the idea to evidence, consequences and unfamiliar situations.':rank>=7?'At JHS level, explain causes, effects and applications.':'At Primary level, connect it to familiar everyday situations.'}<br><br><b>Example:</b> ${esc(ex)}`;
  ...
  return `<b>AI Tutor 5.0 • ${esc(l)} • ${esc(subject)}</b><br><br><b>Your question:</b> ${esc(q)}<br><br><b>Answer:</b> ${esc(def)}<br><br><b>How to apply it:</b> Start from the definition, identify the relevant feature or rule, apply it to the facts in the question, and check the result.<br><br><b>Topic example:</b> ${esc(ex)}`;
 }
