...
function populateExamTopics(){let l=$("#examLevel").value,s=$("#examSubject").value;$("#examTopic").innerHTML=(C[l]?.[s]||[]).map(x=>`<option>${esc(x.title)}</option>`).join("")}
...
async function generateExam(){let l=$("#examLevel").value,s=$("#examSubject").value,scope=$("#examScope").value,topic=$("#examTopic").value,count=Number($("#examCount").value)||10,difficulty=$("#examDifficulty").value;let pool=uniquePool(examPool(l,s,scope,topic));if(!pool.length){$("#examStatus").textContent="No questions are available for this selection yet.";return}$("#examStatus").textContent=`Building a ${l} exam with class-level difficulty and unique questions…`;let endpoint=localStorage.getItem("learnova.aiEndpoint")||"/api/ai/exam";let qs=[];try{let r=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({level:l,subject:s,topic:scope==='topic'?topic:'Whole subject',scope,count,difficulty,questions:pool.slice(0,80).map(x=>({question:x.question,answer:x.answer,topic:x.topic}))})});let j=await r.json();let remote=j.exam||j.questions;if(typeof remote==='string'){try{remote=JSON.parse(remote)}catch{remote=null}}if(Array.isArray(remote))qs=remote.map(x=>({question:x.question||x.prompt||'',choices:x.choices||x.options||[],correct:Number.isInteger(x.correct)?x.correct:0,answer:x.answer||x.explanation||'',hint:x.hint||'',topic:x.topic||topic})).filter(x=>x.question&&x.choices.length>=2).slice(0,count)}catch(e){}if(qs.length<count){let shuffled=pool.slice().sort(()=>Math.random()-.5);let chosen=[];for(const q of shuffled){if(chosen.length>=count)break;if(!chosen.some(x=>x.question===q.question))chosen.push(q)}qs=chosen.map(q=>makeExamChoices(q,pool));}qs=qs.slice(0,count);state.exam={level:l,subject:s,scope,topic,count,difficulty,questions:qs,index:0,score:0,answers:[]};$("#examSetup").classList.add("hidden");$("#examRun").classList.remove("hidden");$("#examStatus").textContent="";showPage("exam");renderExamQuestion()}

function renderExamQuestion(){let e=state.exam,q=e.questions[e.index];let opts=q.choices.map((x,i)=>`<button data-i="${i}">${esc(x)}</button>`).join("");$("#examRun").innerHTML=`<div class="exam-head"><span class="eyebrow">AI EXAM 2.0 • QUESTION ${e.index+1} OF ${e.questions.length}</span><span class="badge">${esc(e.difficulty)} • ${esc(e.subject)}</span></div><div class="quiz-question"><h2>${esc(q.question)}</h2><div class="quiz-options exam-options-list">${opts}</div><p class="hint">💡 Hint: ${esc(q.hint||"Think carefully before choosing.")}</p></div>`;$$('#examRun .exam-options-list button').forEach(b=>b.onclick=()=>answerExam(Number(b.dataset.i)))}
function answerExam(choice){let e=state.exam,q=e.questions[e.index],ok=choice===q.correct;e.answers.push({question:q.question,chosen:q.choices[choice],correct:q.answer,ok,topic:q.topic});if(ok)e.score++;e.index++;e.index<e.questions.length?renderExamQuestion():finishExam()}
function finishExam(){let e=state.exam,u=state.user||getUser(),pct=Math.round(e.score/e.questions.length*100);u.exams=(u.exams||0)+1;u.examScoreTotal=(u.examScoreTotal||0)+pct;u.examHistory=u.examHistory||[];u.examHistory.unshift({at:new Date().toLocaleString(),level:e.level,subject:e.subject,topic:e.scope==="topic"?e.topic:"Whole subject",score:e.score,total:e.questions.length,pct,difficulty:e.difficulty});u.examHistory=u.examHistory.slice(0,20);u.mistakes=u.mistakes||[];e.answers.filter(x=>!x.ok).slice(0,10).forEach(x=>u.mistakes.unshift({type:"exam",question:x.question,correct:x.correct,topic:x.topic,at:new Date().toLocaleString()}));u.mistakes=u.mistakes.slice(0,50);u.activity=u.activity||[];u.activity.unshift({text:`Finished AI Exam: ${e.subject} — ${e.score}/${e.questions.length} (${pct}%)`,at:new Date().toLocaleString()});u.activity=u.activity.slice(0,8);saveUser(u);renderUser();renderActivity();renderReport?.();updateProgress();let weak=e.answers.filter(x=>!x.ok).map(x=>x.topic).filter(Boolean);let weakText=weak.length?`Focus next on: ${[...new Set(weak)].slice(0,3).join(", ")}.`:`Great work — no missed questions!`;$("#examRun").innerHTML=`<div class="quiz-result"><span class="eyebrow">AI EXAM 2.0 COMPLETE</span><div class="score-circle">${e.score}/${e.questions.length}</div><h2>${pct>=80?"Excellent exam!":pct>=50?"Good effort!":"Keep practising!"}</h2><p>You scored ${pct}%. ${weakText}</p><div class="exam-review"><h3>Review</h3>${e.answers.map((x,i)=>`<div class="exam-review-row"><b>${i+1}. ${esc(x.question)}</b><span class="${x.ok?'exam-correct':'exam-wrong'}">${x.ok?'✓ Correct':'✗ Review'}</span>${x.ok?'':`<small>Correct answer: ${esc(x.correct)}</small>`}</div>`).join("")}</div><button class="primary" id="againExam">New AI Exam</button> <button class="ghost" id="examTutor">Ask AI Tutor</button> <button class="ghost" id="examBack">Back to Dashboard</button></div>`;$("#againExam").onclick=()=>{$("#examSetup").classList.remove("hidden");$("#examRun").classList.add("hidden");showPage("exam")};$("#examBack").onclick=()=>showPage("dashboard");$("#examTutor").onclick=()=>{let first=e.answers.find(x=>!x.ok);showPage("tutor");$("#tutorLevel").value=e.level;populateTutorSubjects();$("#tutorSubject").value=e.subject;$("#tutorTopic").value=first?.topic||e.topic||"";$("#tutorQuestion").value=first?`Explain why the correct answer is: ${first.correct}`:`Help me improve my ${e.subject} exam performance.`;$("#askTutor").click()}}
...
function tutorDirectMath(q){
 ...
 if(!Number.isFinite(r))return '<b>AI Tutor 6.0</b><br><br>I can work this out, but division by zero is not defined. Please check the numbers.';
 return `<b>AI Tutor 6.0 • Step-by-step solution</b><br><br><b>1. Identify:</b> We need to calculate ${a} ${esc(op)} ${b}.<br><b>2. Work it out:</b> ${a} ${esc(op)} ${b} = <b>${r}</b>.<br><b>3. Check:</b> The result is ${r}.<br><br><b>Answer: ${r}</b><br><br>Send the full question if there are more steps, units or a word problem.`;
}
...
function renderSchoolPortal(){
 ...
 if(r==="parent"){
   ...
   document.getElementById("parentSummary").textContent=avg>=80?"Excellent progress. Encourage continued challenge and revision.":avg>=55?"Progress is developing. Encourage regular practice and revision.":"The learner needs more foundation practice. Use AI Tutor 6.0 and Smart Revision.";
 }
...
function subjectTutorAnswer(q,l,s,t){
 ...
 for(const [re,ans] of packs) if(re.test(x)||re.test(topic)) return `<b>AI Tutor 6.0 • ${esc(l)}</b><br><br>${ans}${wantsWhy?'':'<br><br><b>Check yourself:</b> State the definition, give one example and explain how the idea applies to the question.'}`;
 return null;
}
...
function advancedMathTutor(q,l,s,t){
 ...
 if(/^[0-9+*/().%^×÷−\-\s]+$/.test(expr) && /\d/.test(expr)){
  ...
  if(r!==null)return `<b>AI Tutor 6.0 • Step-by-step Maths</b><br><br><b>Question:</b> ${esc(raw)}<br><b>Step 1:</b> Follow the order of operations: brackets, powers, multiplication/division, then addition/subtraction.<br><b>Step 2:</b> Calculate carefully: <b>${esc(expr)}</b>.<br><b>Step 3:</b> Check the result by substituting or estimating.<br><br><b>Answer: ${mathFormat(r)}</b>`;
 }
...
function genericTutorAnswer(q,l,s,t){
 ...
 if(topicObj){const def=topicDefinition(topicObj),examples=topicObj.examples||[],ex=examples[0]||`Apply ${title} to a real ${subject} situation.`, lower=String(q).toLowerCase();
  if(/\b(what is|what are|define|meaning|who is|explain)\b/.test(lower))return `<b>AI Tutor 6.0 • ${esc(l)}</b><br><br><b>Definition:</b> ${esc(def)}<br><br><b>Detailed explanation:</b><br>${textbookExplanation(topicObj,l,subject)}<br><br><b>Topic-matched example:</b><br>${esc(ex)}`;
  if(/\b(why|importance|important|reason|purpose|benefit)\b/.test(lower))return `<b>AI Tutor 6.0</b><br><br><b>Why ${esc(title)} is important</b><br>${esc(title)} is important because it helps learners understand and apply ideas in ${esc(subject)}. ${rank>=10?'At SHS level, connect the idea to evidence, consequences and unfamiliar situations.':rank>=7?'At JHS level, explain causes, effects and applications.':'At Primary level, connect it to familiar everyday situations.'}<br><br><b>Example:</b> ${esc(ex)}`;
  ...
  return `<b>AI Tutor 6.0 • ${esc(l)} • ${esc(subject)}</b><br><br><b>Your question:</b> ${esc(q)}<br><br><b>Answer:</b> ${esc(def)}<br><br><b>How to apply it:</b> Start from the definition, identify the relevant feature or rule, apply it to the facts in the question, and check the result.<br><br><b>Topic example:</b> ${esc(ex)}`;
 }
 const math=tutorDirectMath(q); if(math&&isMath(s))return math;
 return `<b>AI Tutor 6.0</b><br><br><b>Answering your question:</b> ${esc(q)}<br><br>I can answer school questions even when no topic is selected. I will identify the concept, explain the answer directly, show the reasoning or calculation, give a practical example and match the explanation to ${esc(l)}.`;
}
function initTutor(){fillClasses($('#tutorLevel'));populateTutorSubjects();$('#tutorLevel').onchange=()=>{state.level=$('#tutorLevel').value;populateTutorSubjects()};$('#tutorModeSelect').onchange=()=>{$('#tutorStatus').textContent='AI Tutor 6.0 • '+$('#tutorModeSelect').selectedOptions[0].textContent.replace(/^\S+\s*/,'')};$('#askTutor').onclick=askTutor;$('#clearTutor').onclick=()=>{$('#tutorQuestion').value='';$('#tutorAnswer').innerHTML='<div class="ai-icon">🤖</div><div><b>Hi! I\'m your Learnova Tutor.</b><p>Ask me anything about your school work.</p></div>'};$$('.tutor-tips button').forEach(b=>b.onclick=()=>{$('#tutorQuestion').value=b.dataset.q||b.textContent;$('#tutorQuestion').focus()});initTeachMode()}
...
function answerAdaptivePractice(choice){
 ...
 $$('#practiceRun [data-pchoice]').forEach(x=>x.disabled=true);const next=document.createElement('div');next.className='practice-actions';next.innerHTML=`<button class="primary" id="nextPractice">${p.index===9?'Finish practice':'Next question →'}</button><button class="outline" id="askPracticeTutor">Ask AI Tutor 6.0</button>`;box.appendChild(next);
