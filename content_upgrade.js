...
(()=>{
  ...
  function otherExamples(title,subject,l){return [`Definition in context: ${definition(title,subject)}`,`Importance: explain why <b>${title}</b> matters to a learner, family, school, community or the wider world.`,`Cause and effect: describe what can happen when the principle behind <b>${title}</b> is applied correctly or ignored.`,`Real-life application: identify a Ghanaian school, home, community, environmental, workplace or civic situation where <b>${title}</b> can be observed.`,`Process example: describe the main steps involved in using, demonstrating or investigating <b>${title}</b>.`,`Comparison: compare <b>${title}</b> with a closely related idea and explain one important difference.`,`Evidence: give an observation, fact, object, event, text, diagram or result that supports an explanation of <b>${title}</b>.`,`Problem-solving: create a realistic problem involving <b>${title}</b>, propose a solution and justify it.`,`Ghana connection: explain one way <b>${title}</b> relates to Ghanaian life, culture, environment, citizenship, work or development where appropriate.`,`Advanced ${l} example: analyse a new situation involving <b>${title}</b>, use evidence or reasoning, and reach a justified conclusion.`]}
